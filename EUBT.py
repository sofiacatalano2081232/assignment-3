...
class Node:
    def __init__(self, name=None):
        self.name = name
    def leaf(self):
        return self.name is not None #checks if the node has a name
...
def enumerate_trees(leaves):  #enumerates all possible trees that can be formed
    if len(leaves) == 2:
        n1, n2 = leaves
